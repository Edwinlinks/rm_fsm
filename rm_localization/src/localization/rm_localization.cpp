...
RmLocalization::RmLocalization() {
  ...
  nh_private.param("moving_average_num", moving_average_num, 5);
...
void RmLocalization::fuseDetectedTags(
  ...
  filter_x_->input(loc_x / loc_weight_sum);
  filter_y_->input(loc_y / loc_weight_sum);
  filter_yaw_x_->input(loc_yaw_y / loc_weight_sum);
  filter_yaw_y_->input(loc_yaw_x / loc_weight_sum);
  map2base.setOrigin(tf2::Vector3(filter_x_->output(), filter_y_->output(), 0));
  tf2::Quaternion quat_from_yaw;
  quat_from_yaw.setRPY(0.0, 0.0,
                       atan2(filter_yaw_y_->output(), filter_yaw_x_->output()));
  map2base.setRotation(quat_from_yaw);
  ...
  new_map2base_msg.transform = tf2::toMsg(map2base);

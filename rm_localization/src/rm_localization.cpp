...
TagsLocalization::TagsLocalization() {
  ...
  tag_sub_ = nh_private.subscribe<apriltag_ros::AprilTagDetectionArray>(
      "tag_detections", 100, &TagsLocalization::tagCB, this);
  nh_private.param<std::string>("camera_frame", camera_frame_,
                                "ep_camera_frame");
  nh_private.param<std::int32_t>(
      "tags_buff", reinterpret_cast<int &>(tags_buffer_size_), 42);
}

void TagsLocalization::FuseDetectedTags(
  ...
  try {
    tf_listener_.lookupTransform(camera_frame_, "base_link", ros::Time(0),
                                 cam2base);
  ...
  for (const auto &detection : msg->detections) {
    ...
    tf_broadcaster_.sendTransform(
        tf::StampedTransform(cam2tag.inverse(), ros::Time::now(),
                             "tag_" + to_string((int)tag_id), "cam"));
    ...
    // Send current tag location
    tf_broadcaster_.sendTransform(
        tf::StampedTransform(map2base, ros::Time::now(), "map",
  ...
  if (fabs(loc_weight_sum) < 1e-8)
    return;

  // get weighted pose (map -> base)
  tf::Transform map2base;

  map2base.setOrigin(tf::Vector3(filter_x.filter(loc_x / loc_weight_sum),
                                 filter_y.filter(loc_y / loc_weight_sum), 0));
  map2base.setRotation(tf::createQuaternionFromYaw(
      atan2(filter_yaw_y.filter(loc_yaw_y / loc_weight_sum),
            filter_yaw_x.filter(loc_yaw_x / loc_weight_sum))));

  // Send current tag location
  tf_broadcaster_.sendTransform(
      tf::StampedTransform(map2base, ros::Time::now(), "map", "fused_base"));
  // get base -> odom
  ...
  tf_broadcaster_.sendTransform(
      tf::StampedTransform(map2odom_, ros::Time::now(), "map", "odom"));
}

void TagsLocalization::tagCB(
    const apriltag_ros::AprilTagDetectionArray::ConstPtr &msg) {
